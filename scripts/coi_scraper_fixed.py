#!/usr/bin/env python3
"""
Captain of Industry Wiki Scraper - Fixed Version
...
class COIWikiScraper:
    ...
    async def scrape_all_data(self):
        ...
        # Step 2: Extract unique buildings (limit to first 10 for testing)
        unique_buildings = set()
        for recipe in recipes_data:
            if recipe['building']:
                unique_buildings.add(recipe['building'])
        
        # Limit to first 10 buildings for faster testing
        unique_buildings = list(unique_buildings)[:10]
        
        logger.info(f"🏭 Found {len(unique_buildings)} unique buildings (limited to 10 for testing)")
...
"""
        ...
        (self.output_dir / "coi_updated.ts").write_text(main_ts, encoding="utf-8")
        ...
        debug_data = {
            ...
            'recipes': recipes_data[:10]  # Limit recipes for readability
        }
        (self.output_dir / "scraped_data.json").write_text(
            json.dumps(debug_data, indent=2, ensure_ascii=False), 
            encoding="utf-8"
        )
        
        logger.info(f"✅ TypeScript files generated in {self.output_dir.absolute()}!")

    def generate_resources_ts(self) -> str:
        """Generate resources TypeScript code"""
        resources_code = "export const coiResources: Resource[] = [\n"
        
        for resource in sorted(list(self.resources)[:20]):  # Limit for testing
    ...
    def generate_recipes_ts(self, recipes_data: List[Dict]) -> str:
        ...
        for i, recipe in enumerate(recipes_data[:10]):  # Limit for testing
