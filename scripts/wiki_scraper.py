#!/usr/bin/env python3
"""
Captain of Industry Wiki Scraper
Extracts recipes, buildings, and metadata from the COI wiki
"""
...
class COIWikiScraper:
    def __init__(self):
        ...
        os.makedirs("scripts/output/images/buildings", exist_ok=True)
        os.makedirs("scripts/output/images/resources", exist_ok=True)
    ...
    def normalize_name(self, name: str) -> str:
        """Normalize names for consistent IDs"""
        if not name or name == "0":
            return ""
        
        # Clean the name and convert to ID format
    ...
    def parse_recipes_table(self, html_content: str) -> List[Dict]:
        """Parse the recipes table from the wiki"""
        logger.info("🔍 Parsing recipes table...")
        ...
        for i, row in enumerate(rows):
            ...
            recipe_data = {
                ...
                'buildingIcon': cells[3].get_text(strip=True),
            ...
            # Parse inputs (up to 6)
            for j in range(6):
                base_idx = 7 + (j * 3)  # Input1Icon starts at index 7
                if base_idx + 2 < len(cells):
                    icon = cells[base_idx].get_text(strip=True)
            ...
            # Parse outputs (up to 6)
            for j in range(6):
                base_idx = 26 + (j * 3)  # Output1Icon starts at index 26
                if base_idx + 2 < len(cells):
                    icon = cells[base_idx].get_text(strip=True)
                    name = cells[base_idx + 1].get_text(strip=True)
                    qty = cells[base_idx + 2].get_text(strip=True)
            ...
            if recipe_data['building'] and (recipe_data['inputs'] or recipe_data['outputs']):
                recipes_data.append(recipe_data)
                
        logger.info(f"✅ Parsed {len(recipes_data)} recipes")
        return recipes_data

    async def scrape_building_metadata(self, building_name: str, semaphore: asyncio.Semaphore) -> Dict:
        """Scrape metadata for a specific building"""
        async with semaphore:
            try:
                # Convert building name to wiki URL format
                wiki_name = building_name.replace(" ", "_")
    ...
    async def download_image(self, image_url: str, filename: str, semaphore: asyncio.Semaphore) -> bool:
    ...
    async def scrape_all_data(self):
        ...
        async with self.session.get(self.recipes_url) as response:
            ...
            recipes_data = self.parse_recipes_table(html_content)
        
        # Step 2: Extract unique buildings
        ...
        # Step 3: Scrape building metadata in parallel
        ...
        await self.generate_typescript_files(recipes_data)
        
        logger.info("🎉 Scraping completed successfully!")

    async def generate_typescript_files(self, recipes_data: List[Dict]):
        """Generate TypeScript files with the scraped data"""
...
"""
        ...
        # Write files
        with open("scripts/output/coi_updated.ts", "w", encoding="utf-8") as f:
            f.write(main_ts)
        
        # Also write separate files for easier inspection
        with open("scripts/output/resources.ts", "w", encoding="utf-8") as f:
            f.write(resources_ts)
        
        with open("scripts/output/buildings.ts", "w", encoding="utf-8") as f:
            f.write(buildings_ts)
        
        with open("scripts/output/recipes.ts", "w", encoding="utf-8") as f:
            f.write(recipes_ts)
        
        # Write JSON for debugging
        with open("scripts/output/scraped_data.json", "w", encoding="utf-8") as f:
            json.dump({
        ...
        logger.info("✅ TypeScript files generated!")

    def generate_resources_ts(self) -> str:
        """Generate resources TypeScript code"""
    ...
    def generate_buildings_ts(self) -> str:
        """Generate buildings TypeScript code"""
    ...
    def generate_recipes_ts(self, recipes_data: List[Dict]) -> str:
        """Generate recipes TypeScript code"""
        ...
        for i, recipe in enumerate(recipes_data):
            ...
            for inp in recipe['inputs']:
                ...
                recipe_code += f'      {{ resourceId: "{resource_id}", amount: {inp["quantity"]} }},\n'
            recipe_code += '    ],\n'
            ...
            for out in recipe['outputs']:
                ...
                recipe_code += f'      {{ resourceId: "{resource_id}", amount: {out["quantity"]} }},\n'
            recipe_code += '    ],\n'
...
async def main():
    ...
    print("✨ All done! Check the 'scripts/output' folder for results.")
